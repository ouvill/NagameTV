...
同日の追加変更で、TS受信・読み出し・正規化・字幕解析のコピーも削減した。
...
## TS解析のコピー削減（2026-09-23追記）

受信から解析までの読み取り専用処理を借用に変更した。連続したバイト列とFFIの受け渡しには
`Vec<u8>`を使い、古いチャンクの破棄には既存の`VecDeque`を使う。バイト列自体を
`VecDeque<u8>`にすると、リングの折り返しで領域が二分され、連続領域を要求するAPIへ
渡す際の並べ替えが必要になる。今回は用途に応じて両者を組み合わせた。

| 箇所 | 変更後の所有とコピー |
| --- | --- |
| HTTP受信 | 完全なパケットは受信バッファーを参照。境界をまたぐ1パケットだけ188バイトの固定領域へコピー |
| メモリー保持 | 最大47 KiBの`Arc<Vec<u8>>`を`VecDeque`で管理。保持領域への書き込みで1回コピーし、読み出しは共有参照と範囲だけを渡す |
| tsreadex | C++の出力領域を借用し、最終出力の`Vec`へ直接追記。各188バイトの処理で作っていた一時`Vec`と追加コピーを除去 |
| 字幕解析 | 完全なパケットは入力を直接参照。未完パケットの端数だけ保持し、次の入力とまたがるパケットを組み立てる |
| GStreamerへの引き渡し | 既存の`Buffer::from_mut_slice`で最終出力の所有権を移す。ここでデータ本体のコピーは発生しない |

保持領域の読み出し後はロックを解放してから解析する。受信側は共有中のチャンクを書き換えず、
次の小さなチャンクへ追記する。`Arc::make_mut`による暗黙の全体コピーは使わない。
通常の追記では読み取り参照がなければ同じ領域を使い、チャンクを毎回作り直すことも避ける。
チャンクが増えても読み出し位置の検索が線形に増えないよう、論理オフセットの二分探索を使う。

保持容量は未使用部分を含むバイト領域の確保量で制限する。破棄された領域でも解析中の
Readerが最大1チャンク、47 KiBを参照する場合がある。Readerは正規化が終わると元TSの
参照を解放し、appsrcへ共有参照を持ち越さない。チャンク管理情報・索引・正規化後の出力は
このTS保持容量とは別である。FS保持の約1 MiBの領域管理とファイル読み出しは維持した。

tsreadexの出力参照はFilterの可変借用に結び付けた。参照を使っている間の再投入を
コンパイルエラーにする試験で寿命の制約を確認する。内容を書き換える正規化処理や、
複数パケットにまたがるPSI／字幕PESの組み立てにはコピーが残る。

### 正規化処理の確保回数

同じ合成MPEG-2 TSを、変更前後の`tsreadex`クレートの`filter` exampleへ188バイトずつ
渡し、heaptrackで比較した。プロセス全体の確保呼び出しは363,097回から30回へ減った。
ヒープの同時使用量のピークは88.71 kBから88.06 kBで、確保回数の削減量とは大きく異なる。
これは正規化部分だけの測定であり、Qtや映像デコーダーを含むアプリ全体の測定値ではない。

同じ測定には、変更前後のexampleを別々に保存して次のコマンドを使う。表示・GPU・音声は不要。

```sh
CARGO_TARGET_DIR=build/cargo cargo build \
  --manifest-path rust/crates/tsreadex/Cargo.toml --release --locked --example filter
heaptrack --record-only -o benchmark/filter-profile \
  build/cargo/release/examples/filter < /path/to/interlaced.ts > /dev/null
```

変更前後の出力は、シーク用・字幕追加用・1080i MPEG-2の3素材で全バイトが一致した。
字幕追加用素材について、1バイト・188バイト前後・47 KiBの入力分割でも、実際の
libaribcaptionで45件の字幕が同じ順番に復号されることを確認した。
記録は無視対象の`benchmark/ts-copy-20260923/`にある。

### TS変更前後の再生中メモリー

先の改善後のバイナリーを今回の変更前として、同じ1080i MPEG-2素材、YADIF、
タイムシフト無効の条件で各3回、35秒ずつ交互に測定した。最初の10秒を除いた
各回の中央値から、3回の中央値を求めた。CPUは1論理CPUを100%とする区間平均の中央値。

| 条件 | RSS MiB | PSS MiB | USS MiB | CPU % |
| --- | ---: | ---: | ---: | ---: |
| TS変更前 | 331.6 | 289.1 | 278.7 | 48.9 |
| TS変更後 | 332.6 | 289.9 | 279.5 | 48.6 |

RSSの範囲は変更前331.2〜338.2 MiB、変更後329.6〜337.1 MiBで重なる。
両方とも約60 fps、安定区間の破棄0フレーム。今回の素材ではアプリ全体の
常駐メモリーとCPUに大きな改善は確認できず、TSの確保回数削減だけでは
...
測定結果・ビルド情報・ハッシュは`benchmark/ts-copy-20260923/playback/`に保存した。
Rustテスト306件、追加した字幕の分割入力試験、tsreadexの2件とコンパイル失敗試験、
通常構成・`native_tests`構成のClippy、Releaseビルド、整形・差分検査が成功した。
専用画面・実GPU・仮想音声出力の検証後、製品Main.qmlのタイムシフト試験と
録画PID変更試験も成功した。前者はメモリー／FS／無効、期限切れからの復帰、容量縮小、
保存方式の変更を確認し、後者はPID変更後も149フレームを観測して正常にEOFへ到達した。
