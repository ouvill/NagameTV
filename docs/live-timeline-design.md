...
| 番組の区間 | 過去番組と放送中番組の開始・終了 | 境界線の上に番組名、下に放送時刻。ホバーでは秒単位の時刻と番組名 |
| 視聴中の番組と位置 | 表示中の映像が、その番組のどの位置か | 区間タイトルを「視聴中 · 番組名」の白い太字にし、カーソルで位置を示す |

番組の進捗には「放送側の進捗」と「視聴位置」がある。白線は放送側の進捗に固定し、
過去番組へ移動しても意味を変えない。視聴位置はカーソルと時刻で示す。
経過時間は実際に視聴した割合ではない。途中をシークしても視聴済みとは扱わない。

放送中と視聴中が同じ番組なら「視聴中」にまとめる。異なる場合は「視聴中」「放送中」を各区間に明示し、
番組詳細を開く操作は視聴中の番組を対象にする。「ライブに戻る」と遅れ時間も維持する。
色だけに依存せず、マーカーの形、ラベル、ホバー表示でも区別する。二本目のシークバーは作らない。

### Penpotの番組跨ぎデザイン（2026-09-19）

「NagameTV · デザイン更新 2026-09-19」の「タイムシフト / 3番組をまたいで視聴」と
「タイムシフト / 番組跨ぎ · 900 × 560」を参照する。通常表示は番組名・バー・時刻の3段とし、
別行の視聴経過時間・放送中タイトル・保持開始の説明ラベルを区間表示へまとめる。

番組名は区間開始から4px内側、12pxの文字。視聴中は白・DemiBold、その他は灰色とする。
軸は4px、保持帯は8px（ホバー・ドラッグ中10px）、放送進捗は3pxで中心を揃える。
再生カーソルは直径16px、LIVEは8pxの菱形、番組境界は高さ18pxの縦線とする。
下には10pxの文字で軸開始・各番組境界・軸終了の時刻と「LIVE 時刻」を配置する。
境界の放送時刻はバックエンドの時計区間から取得し、時計補正をまたぐ軸の単純な比例計算では求めない。

区間が90px未満なら番組名を省略し、長い名称は末尾を省略する。
時刻が隣の境界やLIVEと重なる場合は間引き、LIVEを優先する。ホバーの時刻と番組名は残す。
番組情報未取得時も同じ高さを確保し、利用可能区間があればシークできる。
停止・シーク・エラーは上に状態行を追加し、軸外の停止位置は時刻と番組名を示す別マーカーで表す。
ライブへ戻っても番組区間の位置を保ち、「視聴中」の強調と再生カーソルだけを更新する。

専用GUI環境・実GPUでQML全体253件成功、評価専用18件をスキップ。
ライブ復帰時の区間維持、時計補正を含む境界時刻、狭い幅・左右反転での時刻の重なりを検証した。
ドラッグ中の軸固定、古いセッションの操作拒否、保持範囲外・欠落区間への移動拒否も成功した。
3番組のスナップショットを製品の`LiveTimeline.qml`に与え、1440・900・580px幅、ライブ復帰、
保持範囲外の一時停止を描画確認した。画像とログはGit対象外の`build/timeline-design-review/`、
全体試験のログは`build/timeline-design-qml-tests.log`に保存する。
CMakeの配布用ビルドも成功。ローカル放送フィクスチャーを再生した製品画面で、1440×900と
900×560（再生設定サイドパネルの開閉を含む）の下部配置を確認した。
...
### 番組境界の表示時刻の揺れ（2026-09-19）

視聴中・放送中の番組区間だけを毎回のPCR位置で時刻変換し直し、表示軸と区間一覧は
固定した時計を使っていた。小数ミリ秒を含むPCRではUTCと再生位置の丸め方がずれ、
番組境界が1ms前へ移ることがあった。19:00終了が18:59:59.999になると、分表示が18:59へ切り替わる。

番組区間・表示軸・区間一覧を同じ時計区間の変換に揃えた。開始・終了時刻は番組の予定どおりに
保ち、番組延長や時計の不連続は既存の更新規則で反映する。映像のUTCや実況コメントの時刻変換は
ナノ秒精度のカタログを引き続き使う。

受信開始後に放送時計を取得し、PCRの小数部分を変えて更新する回帰テストを追加した。
開始・終了時刻と番組区間の一致を確認し、製品Main.qmlの通知試験にも終了予定時刻との一致を追加した。
停止で軸を解放する際の境界ラベルもnullを確認して破棄し、停止・選局時の表示警告を防ぐ。
機器不要のRustテスト251件（3件明示的除外）、専用GUI環境でのLiveTimelineテスト15件、
Qt通知・接続試験が成功した。ログはGit対象外の`build/timeline-clock-*-tests.log`に保存する。
製品Main.qmlの起動・録画・タイムシフト試験も成功。メモリー／ファイル方式の一時停止・
巻き戻し・ライブ復帰・容量変更・保持期限切れ後の再開を通じて、終了予定時刻との一致を確認した。
通常配布設定のCMakeビルドも成功し、`build/nagametv`へ反映した。
