...
## 少量の横スクロールコメントを上へ寄せる（2026-09-19）

「均等」の横スクロールは、直近3秒に時間枠へ採用した横スクロールの件数を
使う行数の目安にする。少量なら上の数行、多量なら下へ広がる。
投入量が減っても表示中の行は動かさず、新着だけを上へ戻す。
固定コメントと見送り分は投入量から除外し、文字幅・占有行・相対速度は判断に使わない。
シークでは復元対象より前の投入履歴も読み、最も古い表示対象の配置も再現する。
ランダム・噴水・固定コメントの配置範囲は変更しない。

- 少量・混雑・減少時の配置、見送り分と固定コメントの除外、文字幅への非依存、
...
横スクロールの全画面専用の8秒設定を廃止し、通常・全画面とも5秒を基準にする。
速度倍率を共通で適用する。通常配置は映像幅を移動する時間、評価用の従来配置は
文字全体が退出するまでの時間を指す。上下固定の表示時間は従来どおり。
