...
AppImage／Flatpakのビルドを実行する。`main`へのpush、pull request、`v*`タグのpush、
...
| Tests and AppImage (x86_64) | アプリのRust書式、Rustテスト、独立crateのテスト、Pythonテスト、機器不要のQt試験、AppImageの生成と全ELFのglibc上限検査 |
...
FlatpakはマニフェストのKDE 6.10 SDKを使う。GitHubの実行ホストは両方とも`ubuntu-24.04`。
...
パッケージ単体の作成方法は[AppImage](appimage.md)・[Flatpak](flatpak.md)を参照。
...
5. タグのコミットでCIを再実行し、全ジョブ成功後にGitHub Releaseの**下書き**を作る。
   両パッケージとSHA-256の計4ファイルを添付し、変更履歴を自動生成する。
...
`main`の保護ルールを設定する場合は、上記3つのCIジョブを必須チェックに指定する。
...
bash -n scripts/ci.sh scripts/create-release-draft.sh scripts/build-appimage.sh
actionlint
```
