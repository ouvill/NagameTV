...
設定の表示ページに「字幕に常に縁取りを付ける」を追加した。初期値はOFF。
ONにすると放送で縁取りの指定がない文字に黒い縁取りを加え、放送で指定された
縁取りの色・文字の背景・レイアウトは保つ。設定は`subtitle_force_outline`に保存する。
字幕を非表示にしていても事前に設定できる。

表示中の字幕にも即時反映する。既存のCurveRendererを使い、OFFかつ放送側にも
指定がない文字はアウトラインの生成を行わない。スクリーンショットも同じ有効状態・
色・太さで合成する。字幕本文とTSデコーダーの出力は変更しない。
