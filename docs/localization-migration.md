...
## 実アプリの英語表示と最小サイズ（2026-09-07）

Xvfb :99 / llvmpipeの専用画面で、設定から日本語→Englishへ切り替えた。
1440×900でSettings・接続操作・機能設定・停止画面が即時に英語へ更新された。
900×560へ縮小しても設定の上部項目を表示でき、ホイールで下部のEPG、統計、
Open log folderまで到達した。これらのボタンをすべて実行したという記録ではない。
低層のdiagnostics行は日本語のままで、翻訳対応は残る。

同じ900×560でProgram guide、Terrestrial/BS、Todayを確認し、翌日へ進めると
Tue, Sep 8と日付に対応する番組を表示した。予定番組の詳細には視聴ボタンがなく、
Todayへ戻した放送中の詳細にはWatch this programを表示。ボタンを押すと番組表を閉じ、
NHK総合1京都（3209641984）でPipeline PLAYINGとなり、実映像と字幕を確認した。
番組名・説明は受信した日本語のままで、UIの言語設定で翻訳する対象ではない。

閉じるボタンから終了コード0。検証用TOMLにlanguage = "en"が保存された。
英語での全画面の比較、再起動後の英語復元、音声選択・統計の英語レイアウトまでは未検証。
GPU・メモリー評価は含めない。証跡はgit管理外benchmark/virtual-ui/language.logとlanguage-*.png。

画像とmainソースの照合で、mainの番組表下部60pxの案内欄がProgramGuide.qmlにはないと確認。
見た目の移植残として扱う。mainの案内文には矢印・Enter操作が書かれているが、
両実装の番組表に対応する明示的なキー処理は今回のソース検索では見つからなかった。
単に案内文を追加する前に実際の入力動作を確認する必要がある。


## 計測済みカウンターの再翻訳

設定Drawerのdiagnosticsを固定の日本語からBackendの翻訳テンプレートへ移した。
telemetry::FeatureMetricsに直近の購読・待機・受信・EPGタスク・番組件数と停止待ちを保持し、
表示用QStringはその値から作る。未計測はOption::Noneで空表示とする。
言語変更は保存済みの数値の再表示だけで、字幕購読・EPG通信・計測を実行しない。
10秒の計測間隔とログ形式は保持する。履歴や可変長コレクションは追加しない。

[Qtの翻訳用ソース指針](https://doc.qt.io/qt-6/i18n-source-translation.html)と
[QString::arg](https://doc.qt.io/qt-6/qstring.html#arg)に従い、固定テンプレートを先に翻訳して
数値を差し込む。受信数はu64のまま文字列化し、Qtのintへの縮小変換を行わない。
停止待ちもYes/Noを翻訳する。カタログ試験では6項目の引数、u64最大値の文字列、
両方の停止待ち状態と日英切り替えを確認した。190項目のカタログ生成・既存の
切り替え／欠落試験が成功し、Clippy全ターゲットも警告なし。

releaseビルド後の実アプリでも、専用Xvfb画面の設定下部で英語の診断表示を確認し、
日本語へ切り替えると「停止待ち いいえ」を含む日本語表示となった。
英語は従来の一行省略表示に収まらず末尾が省略されるため、全項目の可読性改善は残る。
証跡はbenchmark/virtual-ui/metrics-translation.logとmetrics-en.png、metrics-ja.png。

## タイムシフトの位置補正通知

保持期限切れ、設定変更による保持範囲内への移動、設定変更によるライブ復帰の3通知は、
`timeline::Notice`として保持し、Playerの表示境界でBackendカタログを使って翻訳する。
表示中の通知も言語変更に追随する。保持期限切れの日本語は
「保持期限を過ぎたため、再生位置を保持範囲内へ移動しました」とする。
通知と診断詳細はenumで区別し、診断詳細を翻訳キーとして扱わない。
通知はスクリーンショット通知と同じ6秒で自動消去する。同じ通知を再度受け取った場合は
その時点から6秒を数え直し、言語変更では期限を延長しない。既存のPlayerポーリングで
現在の通知だけを期限判定するため、古い期限が後から届いた通知や診断詳細を消すことはない。

機器不要の接続試験で、実Playerの3通知の日英往復、更新通知中の読み取り、
診断詳細の保持、期限直前と期限到達時の消去、同じ通知の再発生による期限更新、
後から届いたエラーの保持、消去後に言語を変更しても通知が復活しないことを確認した。
翻訳カタログの切り替え・欠落試験と、専用GPU・仮想音声環境での製品Main.qml起動試験
（タイムシフトの設定変更・保持期限切れ・通知の自動消去を含む）も成功した。
