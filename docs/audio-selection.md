...
視聴画面の音声ボタンをクリック／タップすると、ミュート・音量スライダー・
音量の数値・音声トラックの一覧を一つのパネルで表示する。ホバーではツールチップのみを
表示し、操作パネルを開閉しない。音声選択用の入れ子のメニューは廃止した。
同じボタン、外側のクリック、Escape、パネルの閉じるボタンで閉じる。
パネルは音声ボタンの上に配置し、画面端に収める。開いている間は視聴操作部を隠さない。

パネル内の操作対象は44px以上とし、キーボードのTab・矢印・Spaceに対応する。
開閉は140ms、選択行の色は100msで変化する。ポインターが外れてもパネルは維持し、
音量はドラッグ中に即時反映、操作の終了時に保存する。キー操作直後のパネル閉鎖でも
保存待ちの値を確定する。音声切り替えはバックエンドの確認後に選択状態を更新する。

閉じる途中の再クリックは開く動作へ反転する。閉じるアニメーション中に別の入力欄を
クリックした場合は、そのフォーカスを保つ。Escapeでは起点のボタンへ戻る。

検証: 専用GPU環境で音声操作15件・音声選択5件が成功した。ホバーで開かないこと、
クリック／タッチ／キー操作、音量保存、画面幅、途中の開閉反転、外側へのフォーカス移動を確認した。
製品Main.qmlの全体起動試験も成功し、音声パネルの表示・ミュート・Escapeを確認した。

以下の移植記録にあるchevronや旧配置は当時の実装であり、現在のUIは本節に従う。
