...
| **`Space`** | 共通再生操作（再生／一時停止） | `Playback`（録画再生中・一時停止可能なライブ視聴中、パネル閉時） |
...
- **Navigation**: テキスト入力中、ポップアップ表示中、録画一覧の表示中は無効。
- **Playback**: Navigationの条件に加え、録画再生中または一時停止可能なライブ視聴中であり、かつ番組表・局一覧が閉じている場合のみ有効。
