...
- **一般動画の制約**: 埋め込み字幕・外部字幕と実況同期は未対応です。詳細は[MP4・MKVの再生](#mp4mkvの再生)を参照してください。
- **未実装機能**:
...
一般動画の埋め込み字幕・外部字幕と実況同期は未対応です。TS内の放送時刻に依存する
番組情報も一般動画には表示しません。CMカットなどで編集した動画の実況には、放送時刻と
動画内の位置を対応づける別の設計が必要です。

AppImageには`isomp4`と`matroska`を追加で同梱します。ネイティブ版・debでは既存の
GStreamer Good Plug-ins、Flatpakではプラットフォーム側のプラグインを使います。
