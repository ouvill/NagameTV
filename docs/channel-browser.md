...
以下は2026-09-19時点の記録。現在の下部一覧の操作は
[下部一覧のカルーセル](#下部一覧のカルーセル2026-09-21)を参照。
...
## 下部一覧のカルーセル（2026-09-21）

下部一覧もサイドパネルと同じ`ChannelWheelArea.qml`を使い、入力の距離に応じて
表示位置を連続的に動かす。カード幅＋間隔（従来370px）の入力を待たず、
`pixelDelta`をそのまま移動距離にする。角度入力はホイール1段につき144pxへ換算する。
大きい方の軸を一つ使い、下部は横方向、サイドパネルは縦方向へ移動する。

下部では中央へ近づくカードを270pxから最大356pxまで滑らかに広げる。
固定幅のListViewの移動座標と見た目を分離し、中央の2枚で追加幅を分け合い、
隣のカードをずらして14pxの間隔を保つ。拡大に伴うレイアウト変化が入力距離へ
逆流する循環を作らない。幅が足りない場合はカードを表示領域内へ収める。
仮想化を維持し、表示領域外には追加幅の半分だけ余裕を持たせる。

スクロール中は補間を挟まず指に追従する。下部のピクセル入力では直近80msの実移動から
速度を求め、終了時に160px/秒以上なら[Flickableの慣性](https://doc.qt.io/qt-6/qml-qtquick-flickable.html#flick-method)へ
引き継ぐ。最大2200px/秒、減速度2400px/秒²とし、慣性が止まってから最寄りのカードを
180msのOutCubicで中央へ寄せる。ドラッグの慣性にも同じ上限・減速度を使う。
ゆっくりした微調整、停止してからの指離し、反転前の速度、端で動かなかった入力は
慣性へ持ち越さない。OSが送る慣性のデルタもそのまま適用し、十分に減速した末尾では
追加の慣性を開始しない。QMLではScrollMomentumを直接識別できないため、速度で判断する。

[Qt WheelHandler](https://doc.qt.io/qt-6/qml-qtquick-wheelhandler.html#activeTimeout-prop)
のactiveを使い、位相情報のある入力ではScrollEndを待つ。位相情報がない場合は
下部のピクセル入力で60ms、角度入力とサイドパネルで160msの無入力を終了とする。
縦横どちらから始めても、ジェスチャー中は同じハンドラーが受け持ち、軸を変えた際の
二重配送を防ぐ。角度情報しか届かないトラックパッドには追加の慣性を適用しない。
サイドパネルには追加の慣性・吸着・候補の拡大を加えない。

位置復元・待機・入力追従・吸着はMotion enumで分ける。新しい入力は吸着を即座に
中断し、保留中の中央寄せも上書きする。慣性中の再入力も即座に追従へ戻り、
キー操作・非表示・位置復元では保留中の速度を破棄する。
左右キーは隣の候補へ滑らかに移動する。
スクロールだけでは再生中の局を変えず、クリックでそのカード、Enterで候補を確定する。
開き直すと選択中の局へ戻る。ウィンドウのサイズ変更後のListViewの位置補正にも追随する。

局名のすぐ右に直径6pxの赤い点を置き、ホバーで「視聴中」と表示する。点滅はさせない。
中央の候補とは独立した`Player.viewing_channel`を使い、実際のライブ再生対象の
サービスIDを現在のカタログへ対応付ける。一時停止・タイムシフト中も印を保ち、
停止・接続待ち・録画ファイル再生中は表示しない。カタログから消えた局には表示せず、
再登場や並べ替えでも同じサービスを示す。新しい通知の前に対応する状態を確定する。

スクロールと下部／サイドパネルのQML試験では、距離比例、斜め入力、方向反転、
軸の切り替え、拡大中のカード間隔、吸着の中断、キー操作、拡大部分へのクリック、
一覧の両端、狭い幅への変更、候補と視聴中の印の独立を確認する。
慣性試験では両方向の移動継続と吸着、狭い幅、端での停止、再入力・キー・再表示での中断、
指離し前の停止と反転、低速入力とOS慣性末尾を想定したデルタ列を検証する。
接続試験は実PlayerとローカルHTTPを使い、u64最大値のサービスID、一時停止、
カタログ更新・消失・再登場、停止と録画再生、通知時の対応関係を検証する。
ピクセル入力のQML試験は共通処理関数へ渡すもので、物理トラックパッドの操作感や
OSからのピクセル／位相イベント配送は未検証。

カルーセルと視聴中表示の検証結果: 関連QML試験37件（初期化・終了を含む）、Rust275件（手動試験5件はignored）、
接続・MPRIS・製品Main.qmlの起動試験が成功。qmllint、fmt、差分検査、CMakeのrelease
ビルドも成功した。幅960／360pxの画像と、中央候補を変更しても視聴中の印が元の局に
残る画像を`build/channel-carousel-review/`に保存して確認した。

慣性追加後は関連QML試験46件（初期化・終了を含む）、qmllint、差分検査、CMakeの
releaseビルドが成功。慣性の追加入力はQMLの共通処理へ渡しており、実機の操作感は未検証。
...
## 開くときの視聴局表示（2026-09-21）

下部一覧と右チャンネルパネルは、開くたびに`Player.viewing_channel`の放送波と局へ
表示位置を合わせる。視聴中のライブ局がない場合は選択局、未選択なら先頭局を使う。
視聴局は同時放送の絞り込みで隠さない。下部は中央へ、右パネルはカード全体が
見える位置へ合わせ、非同期の一覧更新後にも元の局indexから位置を求める。
画面の開き直しや放送波の変更だけでは選局しない。

右パネルにも局名のすぐ右に赤い視聴中の点を表示する。下部と共通の
`WatchingIndicator.qml`を使い、直径6px・色・「視聴中」のツールチップを揃える。
候補のキーボード移動から独立して実際の視聴局を示し、停止時・録画再生中は隠す。
