...
設定画面の「コメント」と再生設定から、1280×720の映像を基準にした文字サイズを14〜72pxで変更できる。
初期値は36px。保存値の上限も72pxとし、以前の設定にある12〜13pxは引き続き読み込む。
実際の描画サイズは「黒帯を除いた表示映像の高さ ÷ 720 × 設定した文字サイズ」を
整数pxに四捨五入して求める（最小1px）。たとえば基準36pxなら、表示映像の高さが
360・720・1080pxのとき18・36・54pxになる。ウィンドウの拡縮、サイドメニューの開閉、
全画面切り替えでも表示中のコメントを再計測し、文字・影・行高へ反映する。
設定値は基準サイズのまま保存し、実際の描画サイズには14〜72pxの制限を適用しない。
...
文字の不透明度は設定画面と再生設定の両方で0〜100%を5%刻みで変更できる。
0%は完全透明とし、保存・再読み込み後も維持する。
...
上記の衝突回避は評価用の従来配置に限る。通常版では表示量の「通常」で毎秒2〜12件に制限し、
時間枠ごとの上限を超えたコメントは見送る。「すべて表示」では件数による間引きを行わず、重なりを許す。
表示量は設定画面と再生設定で変更・保存できる。現在の配置と制限は[コメント表示の仕様](comment-display-redesign.md)を参照。

横流れは右端からすぐ入り、通常・全画面とも5秒を基準にする。
通常配置では映像幅を移動する時間、評価用の従来配置では文字全体が退出するまでの時間を指す。
上下固定は通常4秒／全画面6秒。
表示時間は設定の速度倍率で割る。速度倍率だけを変えた場合、既存コメントの移動速度は変えない。
画面寸法、サイドメニュー開閉、全画面状態の変更では既存コメントを消去しない。
文字サイズが変わらない場合、横流れは変更直前の軌道と速度・表示期限を保ち、
新着との衝突判定もその軌道を使う。表示映像の高さに応じて文字サイズも変わる場合は、
上記の文字サイズ変更と同様に再配置し、表示期限を保つ。
上下固定は変更後の画面中央へ追従する。上下固定の全画面での表示時間は新着から適用する。
...
- 取得窓の更新では既存の表示IDを保ち、重複したログを再投入しない。
  録画・タイムシフトの過去ログは、遅着しても残り寿命がある場合だけ途中位置から表示する。
- ライブ先端を再生中に新しく受信したコメントは、横流れなら右端から表示を始め、
  通信・保存・計測の遅れで表示時間を短縮しない。固定表示と噴水も最初から表示する。
  映像より先の時刻に対応するコメントは、その再生時刻まで待つ。
  接続時の受信履歴、一時停止中・シーク中・巻き戻し視聴中の受信はこの対象にしない。
  新着は保存完了を待たずに表示へ渡し、保存済みの同じコメントとの二重表示を防ぐ。
  一時的な表示情報は保持時間・件数・バイト数で制限し、シーク・再生元変更・表示無効化で解放する。
  保存する投稿時刻は変更せず、巻き戻し後は元の時刻を使って途中位置を復元する。
