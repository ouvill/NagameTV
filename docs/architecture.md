...
番組表はRustの`Gap / Single / Conflict`の照会結果をQtの`GuideModel`へ投影する。
同じサービスの時間帯重複は候補を残して競合として表示し、終了未定も既知の終了と区別する。
本文は不変のスナップショットで共有し、区間ごとには複製しない。閉じるアニメーションの
終了後に番組表モデルを解放し、閉じている間は再投影しない。現在の予定は1秒間隔で照会し、
本文は変更時だけ投影する。競合時には任意の候補を現在番組に選ばない。
番組表の詳細・キー操作・視聴可否も同じ照会結果を使い、実行時にはサーバー世代と
局・区間を再検証する。競合中の操作はチャンネル視聴として提供する。
TSの位置付き番組観測は独立させ、不足項目だけをサービス・イベント・開始の完全一致で補完する。
調査根拠と終了未定の表示境界は[EPGアーキテクチャの見直し](epg-architecture-review.md)を参照。
...
一般動画の字幕は`media_subtitles::Session`が所有する。ネイティブのテキストsinkから受けた
字幕を、TIME segmentで映像と同じ再生位置へ変換してlibassへ渡す。シーク先をまたぐ字幕は
元の開始時刻を保持し、ASSの途中からの描画にも使う。字幕画像は`MediaCaption`で映像と
別に描画するため、映像のGPUメモリー契約を変更しない。

字幕の種類は`SubtitleSession`で排他的に表す。ネイティブの停止が成功してからARIBまたは
一般動画の字幕世代を解放する。外部字幕は8 MiBまでのUTF-8ファイルを別スレッドで検証し、
最新の要求だけを適用する。埋め込み字幕は待機4096パケット・8 MiB、1世代のイベントは
10万件・本文8 MiBまでとする。トラック一覧は`SubtitleModel`で公開し、選択にはストリームIDを使う。

EPGStationの再生要求は、動画と同じ所有者に放送局・録画先頭時刻を保持する。リプレイでも
この情報を引き継ぎ、実況時計の有効範囲は動画の実測長さに限る。一般動画は先頭時刻と
実測長さから`Recording::Whole`の取得範囲を作り、開いた時点で動画全体の過去ログを
取得する。TSは引き続き番組情報から取得範囲を決める。CMカットの区間情報や、
編集ソフト固有の時刻補正は扱わない。
