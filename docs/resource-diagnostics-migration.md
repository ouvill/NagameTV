...
| --- | ---: | ---: | ---: |
| 180〜195 | 90 | 331.05〜334.32 | 60.80〜62.54 |
| 195〜198 | 18 | 331.21〜338.27 | 61.23〜63.37 |
| 198〜201 | 18 | 336.15〜348.83 | 63.65〜71.28 |
| 201〜203 | 12 | 344.84〜357.16 | 72.19〜77.58 |
| 203〜206.34 | 21 | 354.05〜368.02 | 78.70〜86.70 |

206.33分時点では再生中、EPG取得開始・完了とも96件、sample 1238件、
threads 41、FD 76、診断drop・GStreamer critical・捕捉stackはいずれも0。
以前の120分までの横ばいは、その観測区間に限る。3時間超でも安定しているという
結論には拡張できない。増加はRSSだけでなくarena内使用中にも現れた。
この集計値だけで割当元やリークを特定することもできない。

EPG取得は195.8467分に完了し、次の開始は200.8467分。
その間も196分のarena内62.16MiBから200分の68.45MiBへ増加しており、
取得・解析中だけの一時ピークでは説明できない。
この区間の保持番組数は13641、EPG_MEMORYのsnapshot容量は3,738,768 bytes。
後続更新では13566番組・3,717,445 bytesへ減ったが、arena内使用中は引き続き増加した。
字幕と実況が無効の条件でも発生したため、字幕文字キャッシュだけを原因とする
説明にも合わない。映像・Qt・通信・その他の保持領域はまだ切り分けられていない。

同時稼働中の全機能PID 132581は79.51分時点でRSS469.70MiB、arena内76.49MiB。
70〜79.52分の58sampleはそれぞれ462.60〜471.74MiB、73.90〜76.62MiBだった。
放送局・機能・起動時刻・バイナリーが異なり、GPUも共有しているため、
この2プロセスの差をEPG有無の対照実験として扱わない。

Git対象外の`benchmark/gstreamer-critical/current/checkpoint-206m/`と
`benchmark/gpu-all-features-20260907/checkpoint-79m/`へ、capture、summary、ranges、
採取時のsmaps・smaps_rollup・maps・status・statと末尾32KiBのログを保存した。
...
## 映像停止の申告と画面観測（2026-09-07）

利用者よりEPGのみの試験で映像停止の可能性を申告された。
PID 78793の生存と診断・EPG取得の継続を確認し、:0のX11と実GPUを検出・検証した後、
対象PIDが所有するウィンドウ60817426だけを読み取り専用で撮影した。
23:01:41と23:01:55 JSTの画像は全ピクセル一致。その後23:02:57には別の場面へ変わり、
初回との不一致は4,665,326ピクセルだった。自動入力・再起動・選局は行っていない。
常時停止ではなく、停止の申告と整合する同一画像の観測後に再び表示が更新された。
これだけでは停止開始・復帰時刻や、連続してフレームが届いたかは判定できない。

217.50分時点のsampleはplaying=true、EPG開始／完了103件、critical 169件。
playingはパイプラインの状態であり、映像フレームの継続到着を保証しない。
これまでの長時間試験のplaying sampleを、映像の無停止再生の証拠として扱わない。
...
## 実TSからbus警告・診断までの結合確認（2026-09-07）

警告メッセージの直接生成とは別に、TSを本体で再生して診断まで届くことを確認した。
CPUのvideotestsrc（120フレーム、320×240、30fps）をavenc_mpeg2videoとmpegtsmuxで
約4秒・235,564 bytesのTSにした。正常版を複製し、PID 65のpayload開始ではない
パケット1個についてcontinuity_counterを4から6へ変更した。
2ファイルの長さは同じで、差はoffset 72,383の1バイトだけと検査した。
TS・ハッシュ・変更情報をbenchmark/ts-warning-ui/へ保存した。

localhostの専用HTTPサーバーで正常版をservice 1、異常版をservice 2として提供した。
Xvfb :99を検出・検証し、明示fakesink、全追加機能OFF、専用設定・診断保存先で
HEAD 504f4bbの本体を起動した。PgDown/PgUpで正常→異常→正常の順に切り替えた。

| 再生 | 終了時rendered | 累積Warning | 累積連続性異常 | 直近PID |
| --- | ---: | ---: | ---: | --- |
| 正常TS・初回 | 120 | 0 | 0 | null |
| 異常入りTS | 119 | 1 | 1 | 65 |
| 正常TS・再選局 | 120 | 1 | 1 | 65 |

変更したPIDと診断が一致し、正常TSへ戻った後には警告件数が増えなかった。
累積カウンターを選局でリセットしない契約も実際の操作で確認した。
警告はtsdemux自身から発生しており、この試験ではbusへの警告注入を行っていない。
有限TSの末尾で出る「配信が終了しました」は想定したEOS処理であり、
無限のライブ配信が途中で停止したという試験結果ではない。

本体は閉じるボタンから終了コード0、専用HTTPサーバーも終了コード0。
実GPUの長時間プロセスは再起動していない。今回はUI／ネイティブ診断の結合確認で、
GPU・音声出力・メモリー消費の測定ではない。
証跡は同ディレクトリーのintact.ts、corrupt.ts、mutation.json、conditions.json、
server.py、server.log、player.log、events.json、state内の生JSONL。
この試験で警告の取得経路は確認できたが、長時間試験の警告と映像停止・メモリー増加の
因果関係はまだ未特定。なお警告カウンターは本体がbusから取得した分を数えるため、
停止時にbusのflushで破棄されたメッセージまでは含まない。
