...
| EPG時間軸・複数局表示・詳細・現在番組 | QML、epg、epg_events | 現在番組・進行率・詳細、7日分の日付選択、予定番組の詳細を追加。全画面の複数局時間軸と画面外列の解放を追加。mainのジャンル配色を追加。上部ツールバーの配置・操作接続を追加。詳細からの視聴とmainとの全体デザイン一致は未完了 |
