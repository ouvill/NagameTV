...
| EPG全局取得・定期更新・更新失敗時保持 | epg、runtime、viewer-core | 基本取得あり。番組表へは指定日分の全局を投影。旧200件制限を廃止。借用ビューからJSONを直接生成し、全セル分の中間配列を除去 |
...
起動時の組み立ては `player/startup.rs` に分離した。EPG接続、字幕表示、局一覧取得の終了待ちも個別モジュールへ分離済み。Player本体の再生・接続調整、既存unwrapの監査、機能移植後の長時間メモリー／性能検証は引き続き必要。
