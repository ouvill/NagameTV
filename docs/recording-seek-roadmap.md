...
| コマンド | 結果 |
| --- | --- |
| Rust releaseテスト | 174 passed / 3 ignored（実サーバー・手動計測用） |
...
| `scripts/test-startup.sh` | 成功（サーバー未設定・再開・前後/連続シーク・番組追従・終端からの復帰） |
...
- [ ] 指定の実録画で感じられるVLCとの滑らかさの差を再現し、原因を特定・改善する。
  [調査記録](video-statistics.md#録画の滑らかさの調査2026-09-16)では59.94fpsの出力とsinkのdrop 0を確認したが、
  Qtの描画通知には短い遅延が残る。VLCとの同一環境での比較は未実施。
