...
代表ケースに絞った信頼性検証は完了したが、同一サービス内のPID変更で再生が進まなくなる
制約が残る。詳細は末尾の追加検証と[検証記録](recording-seek-verification.md#代表ケースの信頼性検証2026-09-16)を参照。
...
- [x] PID変更の不具合を切り分け、独立した再現試験と既知の制約を記録する。
- [ ] PID変更後も後半の映像・音声へ復旧するか、明確なエラーで終了する。
  現状は前半で進まなくなり、エラーも出ない。入力とストリーム切り替えの追加調査として残す。

追加後のRust releaseテストは178 passed / 3 ignored、通常の`scripts/test-startup.sh`は成功。
`scripts/test-startup.sh recording-pid-change`は失敗を再現し、未対応ケースとしてexit 1を返す。
