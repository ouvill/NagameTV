...
基本のシーク・一時停止・TS番組情報表示を実装した。チェックは検証済みの範囲を示す。
未チェック項目は、実録画・長時間・構成変更の追加検証や詳細な状態表示などの残作業。
現在利用できる機能は[TS録画の再生](recording-playback.md)を参照する。
同一サービス内のPID変更で停止するという初回判定は、描画カウンターのリセットを
見落とした誤判定だった。2026-09-17に訂正し、代表素材の変更前後の再生を確認した。
2026-09-17の共通入力実装で、合成TSのPID変更時の時刻連続性と境界をまたぐシークも確認した。
現在の検証範囲と実放送での残作業は[共通TS入力の実装状況](ts-input-implementation.md)を参照。
詳細は末尾の追加検証と[検証記録](recording-seek-verification.md#代表ケースの信頼性検証2026-09-16)を参照。
...
### ライブと録画の共通入力・将来の振り返り再生

ライブ・録画共通化とメモリー／FS保持を含む具体案は
[TS入力の統合設計案](ts-input-design.md)にまとめる。以下は追加要件の概要。

2026-09-17の追加要件。tsreadexの整形処理を録画専用にせず、ライブと録画で共用する。
将来の[ライブ振り返り再生](backlog.md#ライブ配信の振り返り再生)に備え、受信・蓄積と
再生位置を独立して扱う。共通入力と振り返りを実装済み。[現在のチェックリスト](ts-input-implementation.md)。

```mermaid
flowchart LR
    A[ライブ受信] --> B[上限付きTS保持領域]
    B --> C[共通の読み出し・シーク処理]
    D[録画ファイル] --> C
    C --> E[tsreadexによるTS整形]
    E --> F[映像・音声・字幕の再生]
...
この図は元TSを保持して読み出し時に整形する案。保持機構とGStreamerへの接続方式は
比較して決める。GStreamerにも[ディスク上のリングバッファーを使うタイムシフトの設計](https://gstreamer.freedesktop.org/documentation/additional/design/buffering.html#timeshifting)
があるが、tsreadexによるサイズ変更・PID変更・時刻不連続との組み合わせは別途検証する。

- [x] 読み取り位置・利用可能な時刻範囲・処理世代を共通の入力契約として定義する。
  録画の固定範囲と、ライブの最古位置・最新位置が更新される範囲を区別する。
- [x] 一時停止・シークがライブ受信を停止しない所有関係を設計する。
  再生キューが満杯でも、保持領域への受信は再生待ちに依存させない。
- [x] 戻れる最古位置に必要なPAT/PMT・時刻対応も保持し、期限切れで再生不能な範囲を
  シーク可能として公開しない。再接続時の時刻不連続は別区間として扱う。
- [x] 保持時間・容量の上限、保存先、選局時の破棄と、一時停止中に保持範囲を外れた場合の動作を決める。
- [x] UIに最新位置との差と「ライブに戻る」を用意し、番組情報・字幕を再生位置に合わせる。
  実況の履歴保持・同期は別項目とする。
...
- [x] 現行の`playbin3`で、総再生時間・時間単位のシーク可否・範囲・移動後の位置を調べる。
...
- [x] `FLUSH | KEY_UNIT`を初期候補として、指定時刻との差・前後移動・応答時間を測る。
...
- [x] 整形する方式では元ファイルと出力のバイト位置が一致しないことを前提に、
...
- [x] TSパケット同期、セクション組み立て、サービス選択、解析結果の配信の所有者を整理する。
  既存の`transport`を基礎に、字幕と番組情報で構文処理を共有する。
- [x] 選択サービスを維持しつつ、移動先の有効なPATからPMTを取得して構成を再確定する。
  選択サービスが見つからないときに別サービスへ黙って切り替えない。
- [x] シーク境界でTS断片、PSI/PES断片、continuity counter、旧PAT/PMTの状態を無効化する。
  同一サービスへの`select_service()`やcounter不一致だけにリセットを依存させない。
- [x] flush・segment・バッファー不連続・探索読み取りを、採用した入力方式で観測する。
...
- [x] 録画セッションから現在位置・総再生時間・シーク範囲を取得する。
  未取得・非対応・取得済みを区別し、長さの更新に追従する。
- [x] 絶対位置へのシークと相対移動を共通の検証経路へ集約する。
  範囲外・無効値・ライブ・未準備の要求を処理し、時刻演算のオーバーフローを防ぐ。
- [x] 一時停止・再開・シーク中・終端をenumで表し、シーク後に再生/一時停止の意図を維持する。
  明示的な停止は資源を停止し、終端は再シーク可能なセッションとして扱う。
- [x] 要求受付と完了を分け、現在の操作に対応した完了だけを受理する。
  多重要求は実行中1件と最新の希望位置へ集約し、UIスレッドで完了を待たない。
- [x] 拒否・実行中の失敗・待ち時間超過・停止失敗を区別する。
  拒否だけで先頭から再生し直さず、停止失敗時は必要な旧資源を保持する。
- [x] 字幕の表示・待機キュー・PES・デコーダーの管理状態・PTS対応を同期してリセットする。
  移動先のPAT/PMTと字幕データを再取得し、新しい映像時刻の対応で表示を再開する。
- [x] 放送の字幕管理情報・DRCSが欠ける区間の扱いを決める。
  旧状態を捨て、必要な情報の再送を待つ。管理情報・DRCSの再取得を合成試験で確認した。
  追加の先行読み取りとDRCSビットマップ描画の拡張は対象外とする。
...
- [x] 録画用に現在位置/総再生時間、シーク可否、状態と操作をQtへ公開する。
  通知の最中も値の組み合わせが整合していることを接続テストで確認する。
- [x] 録画用シークバーと時間表示を追加する。長さ不明・操作準備中・非対応の表示を用意する。
  録画全体の再生位置と、番組内の進行率を区別する。
- [x] 初期仕様はドラッグ中に予定時刻を表示し、離したときにシークする。
  定期的な位置取得でつまみを戻さず、ドラッグ中は操作部の自動非表示を抑制する。
- [x] 10秒戻し・30秒送り、一時停止・再開、キーボード操作を追加する。
  テキスト入力中やダイアログ表示中のショートカット競合を防ぐ。
- [x] 一時停止中は映像を保持し、停止画面を重ねない。停止・終端の表示とボタンを整理する。
- [x] 狭い画面・全画面・サイドパネル併用、翻訳・操作説明を更新する。
  QML部品試験、接続テスト、製品Main.qmlの起動テストを行う。
- [x] 既存リモートAPIのPlay/Stopと状態通知の意味を確認し、一時停止等との整合性を保つ。
...
- [x] 自TS・対象サービスのEIT p/fを取得し、現在/次の番組を区別する。
...
- [x] SDTからサービス名・放送事業者名を取得する。局ロゴ抽出はこの段階の必須条件にしない。
...
- [x] 番組の予定開始/終了と、録画内で確認した番組切り替わりを区別する。
...
- [x] 先読み・探索中のEIT受信では表示を切り替えず、映像側の再生位置で番組を選ぶ。
  一時停止中は番組内の進行率も停止する。
- [x] シーク時は、検証済みの該当区間の情報か取得待ちを表示する。
  移動前の番組を残さず、巻き戻しで古い区間の情報を正しく選び直す。
- [x] 録画でも番組名・概要・詳細・ジャンル・放送日時・局名を既存UIへ表示する。
  未取得項目を架空の値で補わず、番組名がない場合はファイル名を表示する。
- [x] 録画の情報をライブの選択局・EPGキャッシュ・設定へ書き戻さない。
...
2026-09-16: [実装方式・比較結果・制約](recording-seek-verification.md)を追加。
`FLUSH | ACCURATE`、seqnumを照合する完了判定、録画用enumと操作capability、字幕再取得、
EIT/SDT/TOT/TDT・ARIB文字列・映像位置に同期した番組表示を実装した。

受け入れ素材は60秒の合成TSと既存の字幕/PSI生成試験。
未チェックの複合項目は部分実装を含む。特にSIの全境界条件・日時不連続、実放送の
音声/PID変更、長時間資源計測、Flatpakを一括して合格とは扱わない。
tsreadex比較は同じ合成TSでのファイル再生とシークまで実施。放送切り替え時の
フリーズについては追加素材での再現・比較が必要。

最終検証結果はこの表へ記録する。

| コマンド | 結果 |
| --- | --- |
| Rust releaseテスト | 178 passed / 3 ignored（実サーバー・手動計測用。代表ケースの追加後） |
...
追加の部品検証は、機器検証済みのQt Quick Test起動口から
`--native-tests screenshots -input rust/qml/tests/tst_PlayerControls.qml`と
`--native-tests screenshots -input rust/qml/tests/tst_RecordingTimeline.qml`を指定した。
この起動口は渡されたQMLを実行する共通runnerで、指定した2部品を個別に検証している。
ライブ/録画の両方を492〜1392pxで検証し、長さ不明ではバーを操作できないことも確認した。

### 操作部の改善と実録画の調査（2026-09-16）

- [x] Penpotに合わせ、円形矢印の中に10/30の秒数を置く。
- [x] 録画操作部の独立した停止ボタンを取り除く。
- [x] シークバーのホバー位置の時刻を表示し、ホバー中は操作部を表示し続ける。
- [x] ボタン操作・狭い画面の配置・ホバーでシークしないこと・左右端・RTL・長さ不明からの復帰を部品試験で確認する。
...
[調査記録](video-statistics.md#録画の滑らかさの調査2026-09-16)は保持する。

### 代表ケースに絞った信頼性検証（2026-09-16）

対象を次の代表ケースに限定し、入力構成の変更が必要な問題は別作業に分ける方針で実施した。
各段階の広い未チェック項目や、長時間・実放送・Flatpakの検証を一括して完了とは扱わない。

- [x] 同一サービスのPID変更と時刻の巻き戻りを含む合成TSを追加し、生成方法と構文検証を残す。
- [x] 同じPAT/PMTへの巻き戻しで旧字幕キューを捨て、flush中の入力を無視し、再取得を確認する。
- [x] 字幕管理情報・DRCSの旧状態を使わず、再送後にデコーダーが復旧することを確認する。
- [x] 映像のDISCONT後に字幕PTSの対応を取り直し、旧時刻の字幕を表示しないことを確認する。
- [x] 時刻が巻き戻るTSを製品Main.qmlで最後まで再生し、既存のシーク・終端復帰試験も再実行する。
- [x] PID変更試験の誤判定を訂正し、カウンターのリセットを考慮した回帰試験を通常起動試験へ加える。
- [x] CPU調査でPID変更前後の映像・音声出力、製品試験で両区間の描画と正常終端を確認する。
- [x] 製品のPID変更後も音声が出力されることを確認する。未選択表示と出力停止は区別する。
- [ ] 整形しない入力で音声トラックが未選択表示になる原因を調べ、表示を修正する。
- [ ] PID変更を含む素材で再生位置が区間内の時刻へ戻る原因を調べ、位置の連続性を確保する。
- [ ] PID変更境界をまたぐ前後シークと、一時停止・再開を検証する。
...
### 保持制限・低速ストレージ・実放送境界の対応（2026-09-17）

- [x] タイムシフトのオン／オフ、保存先、容量と時間の設定・保存。
- [x] ライブ中の設定変更を適用し、有限の保持領域から古いTSを破棄。
- [x] 期限切れの一時停止は再開時に保持範囲へ移動する。
- [x] 録画の先頭・末尾から概算長を公開し、未索引のシーク先を優先探索。
- [x] NHK実録画の字幕／副音声追加境界を調査し、字幕・文字スーパー補完で再生継続。
- [ ] NAS実機で待ち時間を確認し、時計の不連続が多い編集TSで優先探索の対応範囲を広げる。
- [ ] 索引の永続化、全編・長時間の再生と音声の聴取確認。

[現在の制約・比較結果・検証方法](ts-input-implementation.md)を参照。
