...
再生位置を独立して扱う。振り返り機能の実装は後続であり、現時点で利用できるものではない。
...
- [ ] 読み取り位置・利用可能な時刻範囲・処理世代を共通の入力契約として定義する。
  録画の固定範囲と、ライブの最古位置・最新位置が更新される範囲を区別する。
- [ ] 一時停止・シークがライブ受信を停止しない所有関係を設計する。
  再生キューが満杯でも、保持領域への受信は再生待ちに依存させない。
- [ ] 戻れる最古位置に必要なPAT/PMT・時刻対応も保持し、期限切れで再生不能な範囲を
  シーク可能として公開しない。再接続時の時刻不連続は別区間として扱う。
- [ ] 保持時間・容量の上限、保存先、選局時の破棄と、一時停止中に保持範囲を外れた場合の動作を決める。
- [ ] UIに最新位置との差と「ライブに戻る」を用意し、番組情報・字幕を再生位置に合わせる。
  実況の履歴保持・同期は別項目とする。
...
- [x] tsreadexによる事前整形と元TSを比較する。代表素材で位置・シーク・選択表示の改善を確認。
  [比較結果と再現手順](tsreadex-trial.md)。現在の既定入力は変更していないため、上の未完了項目は残す。
...
tsreadexのPID固定を事前変換として試した。製品への組み込み、音声補完、字幕の追加先行探索は未着手。
