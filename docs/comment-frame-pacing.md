...
## MP4のCPU負荷増加（2026-09-26）

`72dff22e`と`5b677ae`を別のworktreeでビルドして比較し、字幕のないMP4でも
透明な字幕画像を繰り返し生成する不具合を確認した。該当処理は`8f72421`の一般動画用
字幕対応で追加された。字幕セッションは字幕トラックのない動画でも作られ、
16ms間隔のポーリングから描画処理を呼ぶ。libassはイベントのないトラックで
画像を返さずに変更通知を返すため、変更通知だけで判定すると毎回キャンバスを生成する。
今回の素材では1280×720×4バイトを毎回ゼロ埋めし、QImageの等値比較も発生していた。
GUIスレッドをGDBで10回観察したところ、9回が字幕画像の生成中、1回がQImageの比較中だった。

修正では、字幕レンダラーが最後に出力した内容を「空／画像あり」で保持し、空の状態が
続く場合は画像を再生成しない。画像ありから空への変化と、シーク・字幕切り替えなどの
明示的な更新では透明画像を返し、以前の字幕を消す。GPUや画面周波数による分岐はない。

利用者が報告したH.264/AACのMP4をローカルへ取得し、同じ冒頭を使った。
映像は960×720、ピクセル縦横比4:3、29.970fps。コメントは無効、字幕機能は通常設定、
映像形式は自動選択とした。専用Wayland画面・実GPU・PipeWire仮想出力を検証し、
再起動後の同じNVIDIAドライバー595.91.07、RTX 4070 Ti、Qt 6.10.2、
GStreamer 1.28.2、libass 0.17.4で測定した。すべてNVDECの`nvh264dec`を使い、
入力はSystemMemoryのNV12、出力はGLMemoryのNV12だった。

両版へ同じ計測処理だけを追加し、約5秒の準備期間後の約15秒を集計した。
旧版→現行版→現行版→旧版、旧版→修正版→修正版→旧版の順で実行し、
ビルドや他の試験は並走させていない。CPU使用率はプロセス全スレッドのCPU時間を
経過時間で割った値で、100%がCPU 1コア分に相当する。以下は各版2回の範囲。
旧版は最初の比較の値を載せ、修正版との比較でも6.5〜6.6%だった。

| 版 | CPU使用率 | 動画更新（fps） | 更新間隔p95（ms） |
| --- | ---: | ---: | ---: |
| `72dff22e` | 6.4〜6.6% | 30.02〜30.03 | 34.8〜36.2 |
| `5b677ae` | 98.6〜99.5% | 29.74〜29.91 | 48.7〜49.6 |
| `5b677ae`＋空字幕の再生成抑制 | 6.9〜7.0% | 29.97〜30.03 | 40.8〜42.2 |

動画更新は、描画に取り込まれて`frameSwapped`を迎えた異なるPTSの頻度。
CPU負荷は旧版に近い水準へ下がったが、更新間隔のp95は旧版より長い。
単一の専用画面での短い測定であり、利用者の60／100／144Hzの複数画面や
物理音声出力での滑らかさ・同期を検証した結果ではない。以前の描画比較には
この字幕処理の負荷も含まれていたため、修正後の性能と区別する。

機器不要の再現テストは修正前に失敗し、修正後に成功した。空の字幕での初回消去、
反復ポーリング、明示的な消去、外部字幕から空の内蔵字幕への切り替えを確認した。
既存のMP4／MKV内蔵字幕、外部SRT／ASS、シーク・トラック選択・入力検証を含む
字幕テスト4件が成功した。計測用の差分・実行ファイル・ログはGit対象外の
`build/cpu-review/`に保存し、製品の計測処理は変更していない。

## 10bit MP4と音声同期の追加修正（2026-09-26）

Xwaylandでは10bit AV1のMP4が、P010のGLテクスチャーをNV12へ変換できず、
`not-negotiated`で停止した。自動選択時はNV12とRGBAを許可し、GL入力に応じて
変換形式を交渉する。CPU処理したTSは転送前にNV12へ変換する経路を維持する。
[映像形式の選択](gpu-video.md)

利用者が報告したWaylandでの読み込み待ちは、調査対象のFNS歌謡祭のMP4では
再現していない。実EPGStationからの再生と、TS→MP4→TS→MP4の切り替えを
Waylandで確認したが、同じ現象の解消を確認した結果ではない。

映像表示については、Qtの画面更新1周期を`render-delay`として通知し、
`LATENCY`メッセージで音声・映像全体の遅延を再計算するよう変更した。
[表示待ち時間の扱い](audio-output.md#画面への表示待ちと同期)

同期の比較には、同時に点滅と音声パルスを出す合成素材を使用した。専用Weston、
実GPU、PipeWire仮想出力を検証し、Qtの`frameSwapped`と仮想出力monitorの
パルス時刻を照合した。TSの点滅PTSはデインターレース後の出力から求めた。
他のビルド・試験を並走させず、冒頭4秒を除く約8秒、各条件8〜9組を集計した。
正の値は映像の遅れ、負の値は映像の先行を表す。

| 素材・表示環境 | 修正前の中央値（ms） | 修正後の中央値（ms） |
| --- | ---: | ---: |
| TS・Xwayland（各2回） | 21.2〜25.5 | 5.9〜7.3 |
| TS・Wayland（各2回） | 2.5〜18.2 | 5.2〜8.1 |
| MP4・Xwayland（各1回） | 23.7 | 4.7 |
| MP4・Wayland（各1回） | −3.9 | −17.9 |

TSは修正後の個々の差が約−9〜20msだった。MP4は約−28〜64msとばらつき、
Waylandでは中央値も映像先行側へ動いた。全条件で同期が改善したとは判断できない。
この測定は物理モニターやスピーカーの遅延を含まない。計測コード・ログ・集計は
Git対象外の`build/playback-repair/`へ保存し、製品には計測コードを含めない。

関連するRustテスト11件、映像アイテムの接続・破棄、Xwaylandの製品画面起動、
両表示方式の`video-processing`・`screenshot-playback`、QMLの静的検査が成功した。
起動試験には10bit HEVCの再生・一時停止・シーク・速度変更・再再生・終端を追加した。
Waylandの起動試験全体は、再生前の`root.active && inputContext.navigationEnabled`の
待機でタイムアウトしたため、成功として扱わない。Waylandの再生確認は、上記の
専用試験と実EPGStationへの接続で行った。
