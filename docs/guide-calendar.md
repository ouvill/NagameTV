...
## 空の局とEPG更新をまたぐ時刻の保持（2026-09-07）

キーボードで未来の番組へ移動し、番組情報のない局を通って次の局へ移ると、
時刻が日付の先頭へ戻る不具合を再現した。未操作と、選択時刻はあるが番組がない状態を
同じcursorProgram=nullで扱い、再び初期時刻を選んでいた。

cursorTimeをnull（未操作）または選択済み時刻として持ち、空の局やEPG更新による
番組削除でcursorProgramがなくなっても保持した時刻から解決するよう修正した。
日付・表示局集合の変更では時刻も解除する。新しいEPGコピーや履歴は追加しない。

Xvfb :99 / llvmpipeで、修正前はlate-2を開くべき操作がearly-2を開いて失敗した。
修正後は空の局でEnterしても詳細を開かず、次の局では保持した時間帯の番組を開く。
EPGで番組が差し替わった場合も同じ時間帯の新しい詳細を開き、日付を前日に戻した場合は
前の日付の時刻を持ち越さないことを、実際のキーイベントで検証した。
QML全体93件成功、日付を戻す条件を明確にした後のProgramGuide対象20件成功、
リリースビルド成功、diff検査成功。証跡はbenchmark/guide-empty-channel/のbefore.txt、
all.txt、final.txt。UI試験であり、GPU・メモリー性能の結果としては使わない。
