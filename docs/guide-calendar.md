...
番組表は今日から7日間を選択でき、複数局の時間軸と予定番組の詳細を表示する。
2026-09-25に時間帯の競合を区間モデルへ移し、番組表のJSON一覧をQtモデルへ置き換えた。
現在のデータ境界・競合表示・視聴判断は[EPGアーキテクチャの見直し](epg-architecture-review.md)を参照。
表示範囲に応じた部品の生成と速度計測は[番組表の表示遅延](guide-performance.md)を参照。
以下には、それ以前の段階的な実装・計測の記録も含む。
...
## 番組領域を優先するレイアウト（2026-09-20）

Penpot「NagameTV · デザイン更新 2026-09-19」の
「番組表 / Mirakurun配色・番組領域優先」と「番組表 / 900 × 560 · 情報量優先」を参照する。
ツールバーは当初56pxとし、右上の共通操作の位置を視聴・設定と揃えるため78pxに変更した。
固定局見出しは40px、時刻欄は36pxとし、60pxの常設フッターをなくす。
操作説明は上部の情報ボタンから開き、取得状況は番組情報が空のときに一覧中央へ表示する。
1段のツールバーでは、番組欄の高さが従来より114px増える。

列幅は172pxを下限に、表示できる局数と利用可能な幅から均等に割り当てる。
1440px幅では8局、900px幅では5局を表示できる（対象の局数が少ない場合はその局数で分割）。
局名は白いロゴ下地の横の2行に収める。番組の通常枠線をなくし、背景色`#E4E7E4`を
横1px・列の左右各1pxの区切りとして見せる。選択・キーボードフォーカスの枠だけを残す。
番組内は開始時刻、番組名、余裕があれば説明の順に表示する。短い番組は時刻と番組名を
1行にし、1行にも満たない番組では文字を隠す。`ProgramLayout` enumでこの3段階を表す。
詳細の横位置と横スクロール量は同じ列幅を参照する。

ヘッダーのデザインは検討中のため、日付・放送種別・戻る操作を保ち、右上に共通の
`ModeNavigation`を配置する。番組表モードを選択状態にして、ライブ・録画・番組表・設定の
各操作をMainの共通処理へ渡す。ライブを押すと番組表を閉じ、録画の選択をキャンセルした
場合は番組表に戻る。必要な幅に応じて日付を7日表示／前後移動へ切り替え、操作部が
1段に収まらない場合だけ122pxの2段構成とする。

時間軸はPenpotと同じ1分3px、30分刻み、時／分の上下配置とする。
「共通部品 / 番組表の時間帯カラー」の8色を、ローカル時刻の3時間ごとに適用する。
...
時刻は白、現在時刻の線とバッジは`#B44B3F`とする。夏時間の日も経過時間の添字から
配色を決めず、各帯のローカル時刻で判断する。局見出しは`#202721`、見出しの区切りは`#465049`。

検証には専用GUI環境を使い、画面サイズ別の表示領域、英語・全放送種別での折り返し、
5分／15分番組の選択、詳細の配置と開閉、日付・キーボード・Shift+ホイール操作を確認する。
描画確認用の1440×900／900×560画像は `build/guide-layout-review/` に保存する。

検証結果: ProgramGuideのQML試験24件、番組表4部品のqmllint、CMakeのreleaseビルド、
接続試験、製品Main.qmlの起動試験が成功。共通モード部品の4操作の通知と、製品画面での
番組表から設定への移動、録画選択のキャンセル、ライブへの復帰を確認した。
録画再生・タイムシフト・初回／設定済み起動・終了も確認した。
ログは `build/guide-penpot-tests.txt`、`build/guide-penpot-connection.log`、
`build/guide-penpot-startup.log`、`build/guide-penpot-build.log` に保存した。
描画結果は `build/guide-layout-review/guide-penpot-1440.png` と `guide-penpot-900.png`。

## 日付選択の角丸と詳細の全文表示（2026-09-20）

日付選択は放送種別などと同じ共通枠を使い、外枠12px・選択枠8pxの角丸に揃える。
コンパクト表示の前後ボタンのホバー背景にも外枠の角丸を適用する。

番組をクリックして開く詳細では、番組名・局名・説明を省略せずに折り返す。
カード内の情報領域だけを縦にスクロールし、ホイール・上下キー・スクロールバーで
最後まで読める。放送中の「この番組を視聴」ボタンは下部に固定する。
視聴要求のエラーが出た場合は、そのメッセージまでスクロールして表示する。
閉じて開き直すと先頭から表示し、スクロール操作で背後の番組表は動かさない。

専用GUI環境で600×460／900×560の長文表示、末尾へのスクロール、視聴ボタンの固定、
エラーの表示、閉じて開き直したときの位置を確認した。QML全体261件が成功し、
通常ビルドに含まれない評価用の18件はスキップ。変更した4部品のqmllintも成功した。
ログは `build/guide-details-qml-suite.log`、描画確認は
`build/guide-layout-review/guide-details-top-1440.png` と `guide-details-bottom-900.png`。
CMakeのreleaseビルドも成功。ログは `build/guide-details-build.log` に保存した。

## EPGの詳細情報と読みやすいカード（2026-09-20）

Mirakurunの[番組APIの型定義](https://github.com/Chinachu/Mirakurun/blob/master/api.d.ts)と
[EPGデコード処理](https://github.com/Chinachu/Mirakurun/blob/master/src/Mirakurun/EPG.ts)に基づき、
概要だけでなく、次の情報を番組詳細へ渡す。

- `extended`: 番組内容・番組内容2・出演者・スタッフなど、配信された見出しと本文。
  見出しを固定しない。Serdeでオブジェクトの出現順に読み、Qtへは見出し／本文の配列を渡す。
- `video`: 映像の符号化方式と解像度。既知の形式はMPEG-2、HD、4Kなどの表記を添える。
- `audios`: 主音声／副音声、言語、音声方式、サンプリング周波数。
  二重音声の複数言語も残す。未指定・未知の周波数（-1）は表示しない。
- `series`: シリーズ名、話数、最終話番号。値がある情報だけを表示する。
- `isFree`と既存の大分類ジャンル: 無料／有料とジャンルのラベル。
  無料／有料が不明な場合はラベルを出さない。

映像・音声はEPGに記載された情報であり、再生中のデコーダーの実測値とは区別する。
未取得の項目を推測で補わず、未知の映像形式・言語は受信した名前、未知の音声方式は
コードを保つ。本文は改行を保ったプレーンテキストとし、HTMLとして解釈しない。
情報が更新されると、開いている同じ番組の詳細も更新する。

カードは最大560×620px、ウィンドウの残りの高さに合わせて縮める。
上部に明示的な閉じるボタン、下部に視聴ボタンを固定し、その間をスクロールする。
日付・時刻、番組名、局名、放送中／予定／終了・長さ・ジャンル・無料／有料のラベル、
概要、見出し付きの詳細の順で読む。映像・音声・シリーズは下部の柔らかい緑のカードにまとめる。
見出しと本文の余白、折り返し、行間を揃え、専門的な情報が概要を押しのけないようにする。
日付をまたぐ番組は終了日も表示する。番組表以外の既存の番組詳細にも共通の詳細部品を使う。

EPGは引き続き一つのスナップショットで保持し、32MiB／50,000件の受信上限は維持する。
番組表へは表示日の情報を投影する。追加文字列と詳細セクションの配列容量を
`EPG_MEMORY`の内訳へ加算し、音声言語の表示名は音声切替画面と共通化する。

検証では、EPGの見出し順・改行・任意項目・未知の映像形式・音声周波数をRustで確認した。
Rust全257件成功（外部データ等を必要とする4件はignored）。QML全262件成功
（評価用18件はスキップ）。長文のスクロール、EPG更新後の詳細差し替え、情報欠落時の
非表示、主副音声と複数言語、HTMLを含む本文のプレーンテキスト表示を確認した。
接続・翻訳切り替え・カタログ欠落の試験も成功。ログは `build/epg-details-rust-tests.log`、
`build/epg-details-qml-suite.log`、`build/epg-details-connection.log`、
`build/epg-details-localization.log`。合成EPGによる表示例は
`build/guide-layout-review/epg-rich-top-1440.png` と `epg-rich-bottom-900.png`。
変更したQML部品の静的検査とCMakeのreleaseビルドも成功。
ビルドログは `build/epg-details-build.log`。
製品Main.qmlの起動試験でも、HTTP fixtureのEPGを取得して詳細カードを開き、出演者・
映像形式・音声周波数・シリーズが届くことを確認した。初回／設定済み起動、モード移動、
録画・タイムシフト・終了まで成功。ログは `build/epg-details-startup-final.log`。

## 開閉・スクロール・視聴局への位置合わせ（2026-09-21）

番組表は設定ページと同じ120msのフェードイン、100msのフェードアウトで開閉する。
`AnimatedPanel.Fade`が表示中の部品を所有し、閉じた時点で操作を無効にして、透明に
なってから解放する。Rust側は従来どおり閉じる要求でEPG投影を解放し、QML側だけが
最後の番組・表示列・状態文をフェード完了まで保持する。途中で開き直した場合は
同じ部品でアニメーションを反転し、選択日を再要求する。終了時は直ちに解放する。
番組表はウィンドウ全幅に置き、右パネルの復元でフェード中の幅を変えない。

開くたびに`Player.viewing_channel`の放送波を選び、その局の列まで横スクロールする。
ライブ視聴対象がない場合は保存された選択局、未選択なら先頭局を使う。視聴局は
同時放送の列整理で隠さず、非同期の局一覧・表示列更新後にも位置を合わせる。
表示位置を変更するだけで選局要求は送らない。

番組表のタッチパッド入力は`ChannelWheelArea.BothAxes`で縦横それぞれのピクセル量に
直接追従し、指離し後は下部一覧と共通の速度上限・減速度で慣性スクロールする。
各軸で独立して速度を求め、片側の端で止まっても他方は減速を続ける。新しい入力、
キー操作、放送波・日付変更、非表示で保留中の速度を破棄する。通常のホイールと
Shift＋ホイールの列移動は維持する。低速入力やOS慣性末尾から追加の慣性を起こさない。

QML試験は表示範囲外の視聴局、遅れて届く局一覧、縦横・斜めの移動継続、端・日付変更・
閉じる操作による停止、開閉の反転を確認する。製品Main.qmlの起動試験では閉じる際の
EPG保持と再オープンを確認する。ピクセル入力の試験は入力処理関数へのデルタ列であり、
物理タッチパッドやOSからのピクセル／位相イベント配送の検証ではない。

検証結果: 関連QML試験119件（初期化・終了を含む）、接続・MPRIS・製品Main.qmlの
起動試験が成功。qmllintとfmtも成功した。専用Xwaylandと実GPUを使い、900／1440pxの
番組表と360pxの右パネルを`build/guide-motion-review/`へ保存して、視聴局の表示位置と
赤い点を確認した。開閉の途中状態・反転と慣性の継続・停止は動作試験で確認している。
