...
番組名・放送時間の変更への追従を早めるため、通知による再取得の最小間隔を
60秒から5秒へ短縮した。購読処理とRefreshGateは同じ定数を使用する。
通知が続いても期限を延長せず、最後の通知も後続通知なしで再取得につなげる。
通常受信中の待ち時間には最大約1秒のpoll待ちとHTTP取得・画面反映の時間が加わる。
再接続のヘッダー待ち中はさらに遅れる場合があり、画面反映を5秒以内に保証しない。
通知のない通常時の定期取得は従来どおり5分間隔。
通知が継続する場合、全番組と局一覧の取得頻度は従来の最大12倍になる。

番組表またはチャンネル一覧を開いたときも、局一覧と（EPG有効時の）番組情報を
再取得する。既存データを先に表示し、取得成功後に最新データへ置き換える。
局一覧は取得中なら重複要求を開始せず、EPGは取得中の要求を既存のboolへ集約して
完了後に一度再取得する。閉じる操作では取得を要求しない。

以下の60秒間隔に関する記述・実測結果は変更前の履歴である。
...
## 実通知から番組表を開いた状態での再取得（2026-09-07）

製品コード7b3a19aのアプリを専用Xvfb :99で起動した。EPGのみ有効、再生停止中、
音声は明示的にfakesinkを指定。UI試験として実施し、GPU・メモリー測定には使わない。
読み取り専用のローカルHTTP中継で実Mirakurunへの要求を記録し、応答本文を
変更せず転送した。実サーバーへの書き込みはない。

イベント接続は1本。中継開始32.786〜156.464秒にprogram通知193件を転送した。
programs要求は22.785、82.789、142.783、203.784秒の計4回、すべて完了。
初回から約60・120・181秒の追加取得であり、5分の定期取得より前に発生している。
観測終了までイベント接続の完了・エラー・再接続はなく、切断後の再同期ではない。
通知が止まった後、観測終了まで余分な再取得はなかった。

アプリ診断でもepg_fetch_started/finishedが4組あり、追加3回はguide_open=true。
追加取得の開始→完了はアプリ起動後60194→61097、120194→121096、
181194→182146ms。保持件数は13744件、診断記録の欠落0件。
最初の2応答を比較すると番組IDの増減はなく、2番組にname/description/genres/
video/audiosが追加されていた。前後の画面は番組表を表示しているが、対象番組の
カード変更を直接確認できる証跡ではない。実通知→取得完了は確認済み、
実通知による表示項目の変更は未確認として、検証用データの画面更新試験と区別する。

QML例外・GStreamer criticalはログになく、アプリは終了コード0。
その後、専用中継をSIGTERMで終了した。これは中継から上流への購読が
アプリ終了直後に閉じたことを証明する試験ではない。
証跡はGit対象外のbenchmark/epg-live-ui/にproxy.py/log、programs-1/2.json、
before/after.png、player.log、診断JSONL、summary.jsonを保存した。

別の直接購読は600秒で予定どおり終了（curl終了コード28）。865191 bytesから
完全なprogram/updateを620件解析でき、未解析の末尾は0文字だった。
extended/final-summary.jsonに記録した。既存の163件の再生試験fixtureは保持している。
