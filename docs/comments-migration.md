...
2026-09-22に固定5秒の再接続を段階的な待機延長へ変更した。
現行の待機・停止条件は[NX-Jikkyoの通信と再試行](nx-jikkyo-network.md)を参照。
...
## 受信後のWebSocket切断と再接続（2026-09-07）

既存のController試験はHTTP 503後の再接続開始までだったため、実際に実況を
受信したWebSocketの切断から、次の接続で受信を再開する試験を追加した。
外部サービスやOSのネットワーク設定は変更せず、127.0.0.1のHTTP/WebSocketを使用する。

履歴1件・履歴終了通知・新着70件を送り、closeハンドシェイクなしでTCP接続を閉じる。
受信タスクが失敗状態になるまでControllerからキューを排出せず待ち、
最初のpollが上限64件を返して旧接続を保持することを確認する。
続くpollで残りを排出し、合計71件の順序と履歴／新着の分類が一致する。
旧タスク終了後にRetryingへ移り、5秒の期限直前はWaiting、期限到達で再接続する。
時計は注入したInstantであり、実時間で5秒待った試験ではない。

再接続でHTTPスレッド取得とWebSocket購読をやり直し、履歴1件・履歴終了通知・
新着1件を送る。今度の履歴がHistory、新着がLiveとなり、状態がReceivingに戻る。
最後に無効化してControllerがis_stoppedになるまで待つ。各I/O・待機は2秒上限とし、
読み取りも送信データも限定する。unwrapは追加せずResultで失敗を返す。

network有効のcrate全18件成功、実サービス用の1件はignored、全ターゲットClippy成功。
製品コードに変更はなく再ビルドは不要。Qt画面での再接続状態表示、実サービスの
障害、長時間の併用負荷を確認した試験ではなく、それらの未確認項目は残す。

## DPlayerに合わせた弾幕表示の拡張（2026-09-08）

8行・同時64件・9秒という描画仕様を更新した。可変行数、混雑時の表示見送り、
横流れ／上固定／下固定、mail由来の色、通常5秒／4秒・全画面8秒／6秒に対応する。
文字幅に比例して画面外で待つ開始位置も廃止した。上記の旧仕様・旧試験は当時の記録。
録画用に再生時刻を渡せるDanmakuTimelineを追加したが、録画再生そのものは未実装。
現在の契約と検証は [danmaku.md](danmaku.md) を参照。

## 弾幕コアをRustへ分離（2026-09-08）

QMLに置いていた行選択・衝突・表示時間・寿命・録画時刻順とシークを
viewer-comments::danmaku::Engineへ移した。DanmakuControllerが型付きQt境界を持ち、
QMLは計測と描画・Qt Quickアニメーションを担当する。詳細は [danmaku.md](danmaku.md)。
QML配列を録画モデルとして保持せず、RustのVec<TimedComment>とDurationで管理する。
行別占有で衝突の走査対象を絞り、生成前に配置を判断し、期限切れIDだけを表示側へ通知する。

## Player UI表示時の弾幕消去を修正（2026-09-08）

番組名のvisible変更でViewportが変わり、Engine::configureが表示中コメントをすべて
消していた。回避領域だけの変更では消去せず、上端40pxを基準とする行番号を保持する。
新着だけが番組名に重なる行を飛ばし、既存コメントは座標・期限・占有を維持する。
UIを隠した際も同じ物理行の占有を確認するので、固定コメントの行が重複しない。
Rustで3形式の寿命と行占有、QMLでUI相当の領域変更を3往復して同じLabelと移動継続を検証。
Rust crate 27件・QML全115件が成功した。

## 上下の余裕を考慮したPlayer UI回避（2026-09-08）

既存コメントも開始位置に追随する動作を復元した。上の番組名と下の操作パネルの座標を
Rustへ渡し、占有行数と画面の上下端から移動可能な量を決める。QMLは3種類の親Itemを
180msで動かし、コメントの横移動・期限・行間隔を維持する。
全行使用中は無理に移動させず、既存表示を優先する。新着はUIと重ならず、移動途中も
画面内に収まる行だけを利用する。完全にUIを避けられない場合の方針はdanmaku.mdに記載。
Rustでは3形式の満杯状態・大きな文字と狭い画面・連続反転中の新着を確認。
実Rust型を使うQMLでは滑らかな上下移動・同じLabelの維持・横移動の継続・下端への収まりを確認。
Rust crate 30件・QML全116件が成功。

## 全国の放送IDによる実況先の解決（2026-09-09）

局名の部分一致と地デジ以外のSID直結を廃止し、`comments/mapping.rs`に
EPGStation由来の地デジ248件・衛星38件の対応表を導入した。
取得済みの`networkId`と`serviceId`を使用し、HTTPのサービスID、局名、
リモコン番号、放送種別文字列からは推測しない。両方の放送IDが必要となる。

NIDが0x7880〜0x7fefの場合は地デジ表をSID、SID−1、SID−2の順に検索する。
地デジ以外はNIDとSIDの組み合わせを衛星表で検索する。未対応は実況先なし。
BSのサブサービス統合やBS4K等の対応も表に従う。受信と勢い表示は同じ解決処理を共有する。
表はコンパイル時に組み込み、起動時の追加ダウンロードは行わない。

出典コミット・更新方法・MITライセンスは`third_party/epgstation/`に記録した。
対応表は番組の同時放送を判定するものではなく、地方独自番組の時間帯には
実況内容と映像が一致しない場合がある。

検証: `cargo test --release --locked features::comments` は9件成功。
地方系列局・独立局、サブチャンネル、NID境界、未知ID、衛星のNID/SID組み合わせ、
受信と勢いの対応一致をCPUテストで確認した。実放送・実実況接続の確認は未実施。

## 実況側の番組名と履歴スクロール（2026-09-09）

チャンネルAPIの`program_present.title`を最大512文字・256局の範囲で保持し、
選択局に対応するタイトルをコメントパネル上部の「NX-Jikkyo側の番組」へ表示する。
未取得・null・未対応局は既存の番組名未取得表示とし、機能無効化・サーバー変更時には
タイトルを解除する。番組情報の反映のため、勢いと共用するAPIの取得間隔を
取得完了から5分後→1分後へ変更した。タイトルは放送EPGとの一致を自動判定しない。

コメント履歴のJSONに受信順の文字列IDを付け、QMLのListModelへ追加・先頭削除を
反映する。最下部だけ新着に追従し、ユーザーのスクロール開始で保留中の追従も解除する。
200件上限で先頭が削除されたときは、表示中のコメントIDと行内オフセットを復元する。
表示中のコメント自体が上限から消えた場合は、残っている最古の行へ移る。
縦スクロールバーは常時表示し、履歴全体に対する位置と表示範囲を示す。

検証: Rustの実況機能10件とAPI解析4件が成功。検出・検証したX11 :0とNVIDIA GPUで
CommentListの4シナリオ（追従、履歴上限、先頭消失・クリア、ホイール操作）および
ProgramSidebarの番組名更新・未取得・プレーンテキスト表示を確認した。
対象QMLのqmllintも成功。実サービスからの番組名表示の通し確認は未実施。


## NX-Jikkyoへの投稿（2026-09-13）

表示だけだった入力欄と鉛筆ボタンに投稿機能を追加した。
標準はCtrl+Enter、設定でEnter送信も有効にできる。日本語変換中の誤送信を防ぐ。
受信とは別に、操作ごとに最大1つの投稿タスクを既存runtimeで実行し、
失敗・結果不明を自動再送しない。詳細と検証は[コメント投稿](comment-posting.md)を参照。

## 障害時の待機共有（2026-09-22）

ライブ受信の再接続は5秒から最大300秒の基準時間へ段階的に延長し、勢い・番組情報の
取得失敗も60秒から最大300秒へ延長する。Retry-Afterと比較した後に0〜30秒の揺らぎを加える。
429はヘッダーがなくても待機し、その期限を受信・勢い取得・投稿接続で共有する。
選局や実況ON/OFFでは期限を解除せず、短い接続の成功だけで失敗回数をリセットしない。

待機状態はアプリ起動中の専用クライアントが保持する。保持期間、恒久的なHTTPエラーの
扱い、検証結果は[NX-Jikkyoの通信と再試行](nx-jikkyo-network.md)にまとめた。

WebSocketのCloseコードとwatchのJSON終了理由も型付きで保持するようにした。
受信側の1000は待機後に現在スレッドを取り直し、1008などの要求拒否は接続先を停止する。
サーバー内部エラーの待機は他のNX通信と共有し、投稿後の終了通知では結果不明と自動再送抑止を維持する。
