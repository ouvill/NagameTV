...
2026-09-19更新。設計要件と実装の対応を記録する。拡張案も含むため、実装範囲は次節を参照。

実況の取得範囲とTS読取りは[取得・保持の再設計](comment-archive-redesign.md)に対応済み。
番組情報による取得計画と実際の映像時計を分離し、全編の背景走査は廃止した。
...
- [Catalog](../rust/src/transport/programs/catalog.rs)が番組観測・連続区間・放送時計を所有する。
  PCRシーク索引を間引いてもSIを保持し、上限を設けた補助探索と再生Readerが同じカタログを更新する。
  ライブではTSの保持先頭と出力確認済みの位置の早い方から、コメントの最大表示期間分も
  含めてSI・時計を保持する。短い受信バッファの破棄で出力待ち映像の情報を失わない。
  追加SI読取りは1回8MiB、初期補助読取りは形式検査・端点込み12MiB、シーク補助は合計32MiB以内。
  初期検査・シーク開始から2秒後は新規探索I/Oを発行せず、次のシークで未着手要求を置き換える。
  補助探索後は追加読取りを止める。OSで実行中の読取りの強制中断は保証しない。
...
- [実況Replay](../rust/src/features/comments/replay.rs)がライブ受信と描画位置を分離し、
  ...
  EITの番組に前後2分を加えた範囲をまとめて取得し、一時ファイルからSQLiteへ逐次取り込む。
  EITがない場合だけUTC固定30分枠を使い、録画全編の走査完了を前提にしない。
  応答総量による打切り・分割再取得は行わない。描画には現在位置周辺だけを読み出す。
  保存済み区間・要求頻度制限を再起動後も維持し、失敗と空の成功を区別する。
  ライブコメントはタイムシフト可能な全区間と先頭16秒前までディスクで保持する。
...
EIT scheduleによる全番組一覧、複数サービスの選択UI、
TSで番組を同定できない場合のEPG予定による補助表示、過去ログの一覧UIは今回の範囲に含めない。
既存のコメント一覧はライブ受信履歴を表示し、過去ログは映像上の弾幕へ接続する。

2026-09-19: `design/comment-archive-fetch-policy`で、まとめ取得・永続キャッシュ・ライブ保持を実装。
DBの取得済み区間、受信完了からの復旧、要求制限、使用量と未使用キャッシュ削除は
[過去ログの取得・保存仕様](comment-archive-fetch-design.md)を参照。
...
再生で読み出すTSと量・時間を制限した補助探索を利用し、全編走査は行わない。
...
取得単位は[過去ログの取得・保存仕様](comment-archive-fetch-design.md)に従い、放送日時の範囲が
録画はEITの番組単位を基本として取得し、情報不足時だけUTC固定30分枠を使う。
...
| 録画ファイル | 範囲が確定した録画は未取得範囲をまとめて取得し、SQLiteへ保存 | 録画内の映像位置に対応する放送日時 |
...
2026-09-19の保持要件更新: タイムシフト可能な範囲の受信済みコメントは削除しない。
件数・本文のメモリー上限は描画用の作業集合に適用し、SQLiteへ保存したデータは
TS保持範囲と復元用の手前分が有効な間、通常キャッシュの整理・削除から保護する。
取得並列数と受信待機領域には独立した上限を設け、保存不能は明示的な状態として通知する。
コメント受信を無効にしていた区間、接続断・受信キューの欠落は不足範囲として残す。
映像の保持・シークをコメント保存の都合で自動的に短縮しない。
具体的な寿命と検証条件は[ライブコメントの保持仕様案](comment-archive-fetch-design.md#ライブコメントはタイムシフト可能な区間を保持する)を参照。
