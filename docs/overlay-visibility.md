...
- `OverlayVisibility.qml`：再生中・表示固定・操作通知を受け、表示状態と単発タイマーを所有する。
  非再生中、EPG表示中、文字入力中、ポップアップ表示中、音量ドラッグ中、
  スクリーンショットボタンのホバー中は隠さない。
  ウィンドウからポインターが出たら、固定中でなければ待機せず非表示にする。
  退出後にホバーなどの固定が解除された場合もその場で隠す。
  ウィンドウ内で固定を解除した時点、または映像領域へ再進入した時点から
  新たに3.2秒を待ち、終了時はタイマーを停止する。
- `pointer_activity.h`：mainのウィンドウイベント監視を移植。QML itemの子として1個だけ作る。
  Qt Quickのhover配送より前にマウス移動・進入・ウィンドウ退出を観測し、イベントは消費しない。
...
共通のアイコンボタンのtipはホバー開始時に待ち時間なしで表示する。
tipの表示期限が切れても、スクリーンショットボタン上では操作部を表示し続ける。
...
2026-09-20: ウィンドウ退出時の即時非表示、固定解除が退出より後に届く場合、
再進入時の再表示、tipの即時表示と表示期限後も続くスクリーンショットボタンの
ホバー固定を追加検証した。QMLテスト268件成功（評価用機能など18件はスキップ）。
ネイティブのポインター通知・接続・製品Main.qmlの起動テスト、リリースビルドも成功。
起動テストでは実際の録画再生中に退出・再進入の通知と操作部の表示状態を確認した。
GUI試験は専用Weston/Xwayland、NVIDIA RTX 4070 TiとPipeWire仮想出力で実行した。
