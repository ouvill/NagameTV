...
impl EpgSnapshot {
    pub fn new(services: Vec<Service>, programs: Vec<Program>, synced_at: u64) -> Self {
...
mod tests {
    ...
    fn replaces_snapshots_and_finds_current_program() {
        let first = EpgSnapshot::new(Vec::new(), vec![program(1_000, 500)], 900);
        assert_eq!(first.current_programs(1_250).len(), 1);

        let second = EpgSnapshot::new(Vec::new(), vec![program(2_000, 500)], 1_900);
    ...
    fn preserves_broadcast_audio_and_accepts_missing_metadata() {
        ...
        let snapshot = EpgSnapshot::new(Vec::new(), vec![program], 1000);
        let current = snapshot.current_programs(1250);
        assert_eq!(current[0].audios[0].langs, ["jpn", "eng"]);
        assert!(snapshot.current_programs(1500).is_empty());
    ...
    fn queries_program_guide_ranges() {
        ...
        assert_eq!(snapshot.programs_between(1_400, 2_100).len(), 2);
    }
}
