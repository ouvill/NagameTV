...
Item {
    ApplicationWindow {
        ...
        TestCase {
            ...
            CommentSubmitPolicy {
                id: commentPolicy
                mode: backend.comment_send_on_enter ? CommentSubmitPolicy.EnterOrControlEnter : CommentSubmitPolicy.ControlEnter
            }
            SettingsPanel {
                id: panel
                targetWindow: host
            ...
            function test_shortcut_help_follows_binding_even_while_popup_disables_keys() {
                ...
                const binding = findChild(shortcuts, "channelsShortcut");
                const row = findChild(panel.contentItem, "shortcutHelp_channelsShortcut");
                compare(binding.enabled, false);
                compare(row.keys, binding.nativeText);
                const original = binding.sequence;
                try {
                    binding.sequence = "Ctrl+K";
                    compare(row.keys, binding.nativeText);
                    verify(row.keys.length > 0);
                    verify(row.text.length > 0);
                } finally { binding.sequence = original; }
                for (const name of ["composerShortcut", "recordingShortcut", "playbackShortcut", "seekBackwardShortcut", "seekForwardShortcut"])
                    verify(findChild(panel.contentItem, "shortcutHelp_" + name) !== null);
                backend.comment_send_on_enter = true;
                compare(commentPolicy.mode, CommentSubmitPolicy.EnterOrControlEnter);
            }
