...
Item {
    ...
    readonly property int visibleChannelCount: Math.max(1, Math.min(rows.length,
        Math.floor(view.width / minimumChannelWidth)))
    readonly property real channelWidth: Math.max(minimumChannelWidth, view.width / visibleChannelCount)
    ...
    function revealCursor() {
        ...
        if (top < view.contentY + root.channelHeaderHeight || top + 24 > view.contentY + view.height)
            view.contentY = Math.max(0, Math.min(Math.max(0, view.contentHeight - view.height), top - channelHeaderHeight))
    }
    ...
    function resetPosition() {
        view.contentY = now >= dayStart && now < dayEnd
            ? Math.max(0, Math.min(view.contentHeight - view.height, channelHeaderHeight + (now - dayStart) / 60000 * pixelsPerMinute - view.height * 0.34)) : 0
    }
    ...
    Item {
        ...
        Repeater {
            ...
            Rectangle {
                ...
                y: root.channelHeaderHeight + index * height - view.contentY
        ...
        Rectangle {
            ...
            visible: root.today && root.currentTimeY - view.contentY >= root.channelHeaderHeight
            y: root.currentTimeY - height / 2 - view.contentY
    ...
    Flickable {
        ...
        Repeater {
            ...
            Rectangle {
                ...
                width: view.contentWidth; height: root.dividerWidth; color: root.timeGridColor
            }
        }
        Repeater {
            model: root.rows
            Loader {
                ...
                width: root.channelWidth; height: view.contentHeight
        ...
        Rectangle {
            ...
            visible: root.today && y >= view.contentY + root.channelHeaderHeight
            z: 12; y: root.currentTimeY
            width: view.contentWidth; height: 2; color: root.currentTimeColor
        }
